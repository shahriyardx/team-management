    ...
    <div className="space-y-3">
      {subcategories.map((sub) => (
        <div key={sub.id} className="border border-border">
          ...
          >
            <CaretDown
              className={`size-3.5 text-muted-foreground/60 transition-transform ${collapsedSubs.has(sub.id) ? "-rotate-90" : ""}`}
            />
...
function LazySubcategoryItems({
  ...
  if (isLoading && items.length === 0) {
    return (
      <span className="size-4 animate-spin rounded-full border-2 border-foreground border-t-transparent mx-auto block my-4" />
    )
  }
