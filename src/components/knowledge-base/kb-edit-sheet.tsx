...
export function KbEditSheet({
  ...
  const [files, setFiles] = useState<SelectedFile[]>(
    item.attachments.map((a) => ({ name: a.name, url: a.url, type: a.type, size: a.size, uploading: false })),
  )
  const [links, setLinks] = useState<LinkEntry[]>(
    item.links.length > 0 ? item.links.map((l) => ({ url: l.url, title: l.title })) : [{ url: "", title: "" }],
  )
  ...
  const handleFileSelect = useCallback(async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setFiles((prev) => [...prev, ...selected.map((f) => ({ file: f, name: f.name, url: "", type: f.type, size: f.size, uploading: false }))])
  ...
  return (
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent className="w-full overflow-y-auto sm:max-w-lg">
        ...
        <form onSubmit={form.handleSubmit(handleSubmit)} className="mt-6 space-y-5 px-6 pb-6">
          <Field>
          ...
          <Field>
            ...
            <div className="space-y-2">
              ...
              <Button type="button" variant="outline" size="sm" onClick={() => fileInputRef.current?.click()}>
                <Paperclip className="mr-1.5 size-3.5" />Choose Files
              </Button>
              {files.length > 0 && (
          ...
          <Field>
            ...
            <div className="space-y-2">
              ...
              <Button type="button" variant="ghost" size="sm" onClick={addLink}>
                <LinkIcon className="mr-1.5 size-3.5" />Add Link
              </Button>
            </div>
          </Field>
