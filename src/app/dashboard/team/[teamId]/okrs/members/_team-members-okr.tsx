...
export function TeamMembersOkr({ teamId }: { teamId: string }) {
  ...
  const { data: objectivesData, isLoading: objectivesLoading } = api.objective.list.useQuery(
    { cycleId: cycleId ?? "", organizationId: organization?.id ?? "", teamId, scope: "member" },
  ...
  const createObjectiveMutation = api.objective.create.useMutation({
    onSuccess: () => {
      utils.objective.list.invalidate({ cycleId: cycleId ?? "", scope: "member" })
  ...
  // Filter team members (exclude leader from member assignments)
  const teamMembers = team?.members ?? []
  const teamLeaders = teamMembers.filter((m) => m.role === "leader")
  const assignableMembers = teamMembers.filter((m) => m.role !== "leader" || teamLeaders.length <= 1)
        ...
        <>
          ...
          <div className="space-y-4">
            {teamMembers.map((member) => {
              ...
              const memberObjs = memberId ? (memberObjectives.get(memberId) ?? []) : []
                  ...
                  <summary className="flex cursor-pointer items-center justify-between px-4 py-3 hover:bg-accent/50">
                    <div className="flex items-center gap-3">
                      <Avatar className="size-7">
                        ...
                        <AvatarFallback className="text-[10px]">{member.user.name.charAt(0)}</AvatarFallback>
                      </Avatar>
                      <div>
                        ...
                        {member.role === "leader" && (
                          <Badge variant="outline" className="ml-2 text-[9px]">Leader</Badge>
                        )}
                      </div>
                    </div>
                    <div className="flex items-center gap-4">
                      ...
                      <span className="w-8 text-right text-xs tabular-nums text-muted-foreground">{avgProgress}%</span>
                    </div>
                  </summary>
                  <div className="border-t border-border px-4 py-3">
                    {memberObjs.length === 0 ? (
                      <p className="text-xs text-muted-foreground text-center py-2">No objectives assigned yet.</p>
                    ) : (
                      <div className="space-y-3">
                        {memberObjs.map((obj) => (
                          <ObjectiveCardWithKRs
                            ...
                            onAddKr={openKrForm}
                            onEditObjective={(id, title) => updateObjectiveMutation.mutate({ id, title })}
      ...
      <Dialog open={objFormOpen} onOpenChange={setObjFormOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>Create a member-level objective. This member can check in on progress.</DialogDescription>
          </DialogHeader>
          <form onSubmit={handleCreateObjective}>
            <div className="space-y-4">
              <Field>
                <FieldLabel>Title</FieldLabel>
                <Controller control={objectiveForm.control} name="title" render={({ field }) => <Input {...field} placeholder="Complete onboarding flow" />} />
              ...
              <Field>
                ...
                <Controller control={objectiveForm.control} name="ownerId" render={({ field }) => (
                  <Select value={field.value} onValueChange={field.onChange}>
                    <SelectTrigger className="h-8 w-full rounded-none text-xs">
                      {teamMembers.find((m) => memberByUserId.get(m.userId) === field.value)?.user.name ?? "Select member"}
                    </SelectTrigger>
                    <SelectContent position="popper">
                      {teamMembers.map((m) => (
                        <SelectItem key={m.id} value={memberByUserId.get(m.userId) ?? ""}>
                          <div className="flex items-center gap-2">
