...
export default function TeamCategoriesPage() {
  ...
  const { data, isLoading } = api.knowledgeBase.categoryList.useQuery(
    { organizationId: organization?.id ?? "", teamId },
  ...
  async function handleCategorySubmit(data: CategoryForm) {
    ...
    if (catDialog.edit) {
      ...
      await createCategory.mutateAsync({ organizationId: organization.id, name: data.name, teamId })
    }
    ...
    <div className="flex flex-1 flex-col p-6">
      <div className="mb-6 flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-muted-foreground mt-0.5">Manage team knowledge base categories and subcategories</p>
        </div>
