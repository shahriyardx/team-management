...
import { ArrowLeft, TrashSimple, CloudArrowUp, Paperclip, Link as LinkIcon } from "@phosphor-icons/react"
  ...
  return (
    <div className="flex-1 overflow-auto p-6">
      <div className="mb-8">
        ...
        <p className="mt-0.5 text-xs text-muted-foreground">Deleted knowledge items can be restored or permanently removed.</p>
      </div>
      ...
      {isLoading ? (
        ...
        <div className="space-y-2">
          {items.map((item) => (
            <div key={item.id} className="flex items-center gap-4 rounded-none border border-border px-4 py-3">
              <div className="min-w-0 flex-1">
                ...
                <div className="mt-0.5 flex items-center gap-3 text-[10px] text-muted-foreground">
                  ...
                  {item._count.attachments > 0 && (
                    <span className="flex items-center gap-1"><Paperclip className="size-2.5" />{item._count.attachments}</span>
                  )}
                  {item._count.links > 0 && (
                    <span className="flex items-center gap-1"><LinkIcon className="size-2.5" />{item._count.links}</span>
                  )}
                  {item.subcategory.name && (
                    <span>{item.subcategory.category.name} &rsaquo; {item.subcategory.name}</span>
                  )}
                </div>
              </div>
              <div className="flex shrink-0 gap-2">
                ...
                >
                  <CloudArrowUp className="mr-1 size-3.5" />Restore
                </Button>
                <Button
                  variant="destructive"
                  size="sm"
                ...
                >
                  <TrashSimple className="mr-1 size-3.5" />Delete
                </Button>
              </div>
            </div>
          ))}
      ...
      <Dialog open={!!deleteTarget} onOpenChange={() => setDeleteTarget(null)}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>This will permanently delete this item and its files. This action cannot be undone.</DialogDescription>
          </DialogHeader>
          <DialogFooter>
