...
export default function TeamLeaderDashboardPage() {
  ...
  if (!activeTeamId) {
    return (
      <div className="flex flex-1 items-center justify-center p-6">
        <p className="text-xs text-muted-foreground">No active team selected. Switch teams using the team dropdown.</p>
      </div>
    )
  }
  ...
  return (
    <div className="flex flex-1 flex-col gap-6 p-6">
      <div>
        <h1 className="text-lg font-semibold text-foreground">{team?.team?.name ?? "Team Dashboard"}</h1>
      ...
      <div className="border border-border p-4">
        ...
        {team?.team?.members && team.team.members.length > 0 ? (
          <div className="space-y-2">
            {team.team.members.map((tm: { id: string; user: { id: string; name: string } }) => (
              <div key={tm.id} className="flex items-center gap-2 text-xs">
                <span className="font-medium text-foreground">{tm.user.name}</span>
                {team.team?.leader?.user?.id === tm.user.id && <span className="text-muted-foreground">(leader)</span>}
              </div>
            ))}
          </div>
        ) : (
