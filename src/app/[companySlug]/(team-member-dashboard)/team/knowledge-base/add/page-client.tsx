...
export default function TeamAddKnowledgePage() {
  ...
  const handleFileSelect = useCallback(
    async (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      setFiles((p) => [
        ...p,
        ...selected.map((f) => ({ file: f, name: f.name, uploading: false })),
      ])
  ...
  function addLink() {
    setLinks((p) => [...p, { url: "", title: "" }])
  }
  function updateLink(i: number, f: keyof LinkEntry, v: string) {
    ...
    try {
      ...
      const uploaded = await Promise.all(
        files.map(async (f) => {
          ...
          b.set("file", f.file)
          b.set("type", "knowledgebase")
          const r = await fetch("/api/knowledge/upload", {
            method: "POST",
            body: b,
          })
          if (!r.ok) {
            const e = await r.json().catch(() => ({ error: "Upload failed" }))
            return { ...f, uploading: false, error: e.error }
          }
          const { url } = await r.json()
          return { ...f, uploading: false, url }
        }),
      )
      setFiles(uploaded)
      if (uploaded.filter((f) => !f.url).length > 0) {
      ...
      await createItem.mutateAsync({
        ...
        links: vl.map((l) => ({ url: l.url, title: l.title || l.url })),
      })
      ...
      <div className="max-w-xl">
        <div className="mb-6">
          ...
          <p className="text-xs text-muted-foreground mt-0.5">
            Add a new knowledge base entry for your team.
          </p>
        </div>
