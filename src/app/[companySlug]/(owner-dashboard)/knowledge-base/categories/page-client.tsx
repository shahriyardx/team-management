...
export default function CategoriesPage() {
  ...
  return (
    <div className="flex flex-1 flex-col p-6">
      <div className="mb-6 flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="text-xs text-muted-foreground mt-0.5">
            Manage knowledge base categories and subcategories
          </p>
        </div>
      ...
      <Dialog
        open={catDialog.open}
        onOpenChange={(open) => setCatDialog({ open })}
      >
        <DialogContent>
          <DialogHeader>
            <DialogTitle>
          ...
          <form onSubmit={catForm.handleSubmit(handleCategorySubmit)}>
            <Field>
              ...
              {catForm.formState.errors.name && (
                <FieldError>{catForm.formState.errors.name.message}</FieldError>
              )}
            </Field>
      ...
      <Dialog
        open={subDialog.open}
        onOpenChange={(open) =>
          setSubDialog({ open, categoryId: subDialog.categoryId })
        }
      >
        <DialogContent>
          <DialogHeader>
            <DialogTitle>
              {subDialog.edit ? "Edit Subcategory" : "New Subcategory"}
            </DialogTitle>
            <DialogDescription>
              {subDialog.edit
          ...
          <form onSubmit={subForm.handleSubmit(handleSubcategorySubmit)}>
            ...
            <DialogFooter className="mt-4">
              <Button
                ...
                variant="outline"
                size="sm"
                onClick={() => setSubDialog({ open: false, categoryId: "" })}
              >
                Cancel
              </Button>
              <Button
