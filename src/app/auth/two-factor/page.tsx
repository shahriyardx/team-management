      ...
      <div className="border border-border bg-card p-8">
        ...
        {method === "totp" ? (
          <div className="flex flex-col items-center gap-3">
            ...
            <button type="button" onClick={() => { setMethod("backup"); setError(null) }} className="text-xs text-muted-foreground underline underline-offset-4 hover:text-foreground">
          ...
          <div className="space-y-3">
            ...
            />
            <Button onClick={handleVerify} disabled={!backupCode || verifying} className="w-full">
              {verifying ? "Verifying..." : "Verify backup code"}
            </Button>
            {error && <p className="text-xs text-destructive text-center">{error}</p>}
            <button type="button" onClick={() => { setMethod("totp"); setError(null) }} className="block w-full text-center text-xs text-muted-foreground underline underline-offset-4 hover:text-foreground">
